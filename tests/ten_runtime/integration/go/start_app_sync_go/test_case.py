...
def test_start_app_sync_go():
    ...
    app_dir_name = "start_app_sync_go_app"
    ...
    if sys.platform == "win32":
        ...
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path,
            "start_app_sync_go_app/ten_packages/system/ten_runtime/lib",
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    server_cmd = os.path.join(base_path, "start_app_sync_go_app/bin/start")
    client_cmd = os.path.join(base_path, "start_app_sync_go_app_client")
    ...
    if not is_started:
        print("The start_app_sync_go is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of start_app_sync_go: ", exit_code)
