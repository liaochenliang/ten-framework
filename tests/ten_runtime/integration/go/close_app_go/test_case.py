...
def test_close_app_go():
    ...
    if sys.platform == "win32":
        print("test_close_app_go doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "close_app_go_app/ten_packages/system/ten_runtime/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
        ...
        if (
            ...
            libasan_path = os.path.join(
                base_path,
                (
                    "close_app_go_app/ten_packages/system/"
    ...
    server_cmd = os.path.join(base_path, "close_app_go_app/bin/start")
    client_cmd = os.path.join(base_path, "close_app_go_app_client")
    ...
    if not is_started:
        print("The close_app_go is not started after 30 seconds.")
