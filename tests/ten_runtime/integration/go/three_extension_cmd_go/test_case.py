...
def test_three_extension_cmd_go():
    ...
    app_dir_name = "three_extension_cmd_go_app"
    ...
    if sys.platform == "win32":
        print("test_three_extension_cmd_go doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path,
            "three_extension_cmd_go_app/ten_packages/system/ten_runtime/lib",
        )
    else:
        # client depends on some libraries in the TEN app.
        ...
        if (
            ...
            libasan_path = os.path.join(
                base_path,
                (
                    "three_extension_cmd_go_app/ten_packages/system/"
    ...
    server_cmd = os.path.join(base_path, "three_extension_cmd_go_app/bin/start")
    client_cmd = os.path.join(base_path, "three_extension_cmd_go_app_client")
    ...
    if not is_started:
        ...
        print("The exit code of three_extension_cmd_go: ", exit_code)
