"""
Test start_graph_and_communicate_go.
"""
...
def test_start_graph_and_communicate_go():
    ...
    app_dir_name = "start_graph_and_communicate_go_app"
    ...
    if sys.platform == "win32":
        print("test_start_graph_and_communicate_go doesn't support win32")
        assert False
        ...
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path,
            (
                "start_graph_and_communicate_go_app/"
        ...
        if (
            ...
            libasan_path = os.path.join(
                base_path,
                (
                    "start_graph_and_communicate_go_app/ten_packages/system/"
    ...
    server_cmd = os.path.join(
        base_path, "start_graph_and_communicate_go_app/bin/start"
    )
    client_cmd = os.path.join(
        base_path, "start_graph_and_communicate_go_app_client"
    )
    ...
    if not is_started:
        print(
            "The start_graph_and_communicate_go is not started after 30 seconds."
        )

        server.kill()
        exit_code = server.wait()
        print("The exit code of start_graph_and_communicate_go: ", exit_code)
