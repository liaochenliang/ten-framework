...
def test_prepare_to_stop_go():
    ...
    app_dir_name = "prepare_to_stop_go_app"
    ...
    if sys.platform == "win32":
        print("test_prepare_to_stop_go doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path,
            "prepare_to_stop_go_app/ten_packages/system/ten_runtime/lib",
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    if not is_started:
        print("The prepare_to_stop_go is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of prepare_to_stop_go: ", exit_code)
