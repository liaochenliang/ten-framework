...
def test_multi_dest_go():
    ...
    app_dir_name = "multi_dest_go_app"
    ...
    if sys.platform == "win32":
        print("test_multi_dest_go doesn't support win32")
        assert False
        ...
        if (
            ...
            libasan_path = os.path.join(
                base_path,
                (
                    "multi_dest_go_app/ten_packages/system/"
    ...
    if not is_started:
        print("The multi_dest_go is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of multi_dest_go: ", exit_code)
