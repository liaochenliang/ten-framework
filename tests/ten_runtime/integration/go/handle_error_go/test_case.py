...
def test_handle_error_go():
    ...
    if sys.platform == "win32":
        print("test_handle_error_go doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "handle_error_go_app/ten_packages/system/ten_runtime/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    client_cmd = os.path.join(base_path, "handle_error_go_app_client")
    ...
    if not is_started:
        print("The handle_error_go is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of handle_error_go: ", exit_code)
