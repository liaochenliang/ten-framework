...
def test_local_dependency_1_go():
    ...
    app_dir_name = "local_dependency_1_go_app"
    ...
    if sys.platform == "win32":
        print("test_local_dependency_1_go doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path,
            "local_dependency_1_go_app/ten_packages/system/ten_runtime/lib",
        )
    else:
        # client depends on some libraries in the TEN app.
        ...
        if (
            ...
            libasan_path = os.path.join(
                base_path,
                (
                    "local_dependency_1_go_app/ten_packages/system/"
    ...
    server_cmd = os.path.join(base_path, "local_dependency_1_go_app/bin/start")
    client_cmd = os.path.join(base_path, "local_dependency_1_go_app_client")
    ...
    if not is_started:
        print("The local_dependency_1_go is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of local_dependency_1_go: ", exit_code)
