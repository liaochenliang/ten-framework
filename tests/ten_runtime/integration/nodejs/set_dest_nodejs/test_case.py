...
def test_set_dest_nodejs():
    ...
    app_dir_name = "set_dest_nodejs_app"
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "set_dest_nodejs_app/ten_packages/system/"
    ...
    if not is_started:
        print("The set_dest_nodejs is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of set_dest_nodejs: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print("The set_dest_nodejs can not stop after 30 seconds.")
            server.kill()
