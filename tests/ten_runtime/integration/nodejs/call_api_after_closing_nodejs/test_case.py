"""
Test call_api_after_closing_nodejs.
"""
...
def test_call_api_after_closing_nodejs():
    ...
    app_dir_name = "call_api_after_closing_nodejs_app"
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "call_api_after_closing_nodejs_app/ten_packages/system/"
    ...
    if not is_started:
        print(
            "The call_api_after_closing_nodejs is not started after 30 seconds."
        )

        server.kill()
        exit_code = server.wait()
        print("The exit code of call_api_after_closing_nodejs: ", exit_code)
