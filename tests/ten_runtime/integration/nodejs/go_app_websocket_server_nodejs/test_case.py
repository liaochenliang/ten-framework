"""
Test go_app_websocket_server_nodejs.
"""
...
def test_go_app_websocket_server_nodejs():
    ...
    app_dir_name = "go_app_websocket_server_nodejs_app"
    ...
    if sys.platform == "linux":
        if (
            ...
            libasan_path = os.path.join(
                base_path,
                (
                    "go_app_websocket_server_nodejs_app/ten_packages/system/"
    ...
    server_cmd = os.path.join(
        base_path, "go_app_websocket_server_nodejs_app/bin/start"
    )
    ...
    if not is_started:
        print(
            "The go_app_websocket_server_nodejs is not started after 30 seconds."
        )

        server.kill()
        exit_code = server.wait()
        print("The exit code of go_app_websocket_server_nodejs: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print(
                "The go_app_websocket_server_nodejs can not stop after 30 seconds."
            )
            server.kill()
