...
def test_send_audio_frame_nodejs():
    ...
    app_dir_name = "send_audio_frame_nodejs_app"
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "send_audio_frame_nodejs_app/ten_packages/system/"
    ...
    if not is_started:
        print("The send_audio_frame_nodejs is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of send_audio_frame_nodejs: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print("The send_audio_frame_nodejs can not stop after 30 seconds.")
            server.kill()
